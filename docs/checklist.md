...
- [ ] **A test that your StoreKit configuration file sells exactly those identifiers.** `StoreKitConfiguration(contentsOf:).problems(against:)`, one line in a plain unit test of yours. It also checks that every product is a non-consumable, that a trial is free and not family-shareable, and that an unlock's Family Sharing agrees with the catalogue. [ran] ([App Store Connect](09-app-store-connect.md#check-the-file-against-the-catalogue))
...
- [ ] **The package's own tests run in release as far as they can**: everything that does not need the simulated store, which does not exist there, plus a few tests of `PurchaseStore` itself against a store front that lives in `Tests/` (`make release-tests`; 88 of 206 today), and **`make check` compiles the Demo with its hosted tests for the Mac and iOS, its UI tests, and the app once more in Release, linked**, since nothing else would notice an API change breaking them. [ran]
...
- [ ] **Link `PurchaseTestKit` into test targets and never into the app.** Much of it is in every configuration, so an app that links it ships it; `swift package release-check --app` fails one that does. [ran]
