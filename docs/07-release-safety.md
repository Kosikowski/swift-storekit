...
**Absent, not disabled.** Two modules — `PurchaseTestKit` (`SimulatedStoreFront`, its gates, `Scenario`) and `PurchaseDebugUI` — are wrapped in `#if DEBUG` whole, every file from its first line to its last. In a release build they are empty: not only can nothing in them grant a purchase, there is nothing in them. In a release build those types do not exist. App code that mentions them outside its own `#if DEBUG` does not compile, so the compiler finds the mistake. `#if DEBUG` is also Apple's own pattern for code that belongs to the test environment. **[Apple]**
...
What is not under `#if DEBUG` is in a module of its own, `PurchaseTestSupport`: `ManualClock`, `waitUntil`, `RecordingPurchaseLogger`, and `StoreKitConfiguration` with its problems and errors. They grant nothing, and an app's tests need them in whatever configuration they are built — **but an app does not need them at all**, and Xcode links a package product into every configuration of a target or none. While they shared a module with the simulated store, every app that wanted a debug panel shipped a manual clock. Link `PurchaseTestSupport` into test targets only. (The one reason to link it into an app is `SimulatedStoreFront(catalogue:configuration:)`, for previews with the file's real names and prices; the price is the `.storekit` reader in your release build.)
