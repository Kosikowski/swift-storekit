...
| A trial with five minutes left | **Depends on the OS and the tools.** Apple documents it — `Product.PurchaseOption.purchaseDate(_:)` with `SKTestSession.buyProduct(identifier:options:)` — and it works in the iOS 27 simulator, and on macOS 26.6 with Xcode 26.6. On macOS 26.6 with Xcode **27.0**, `buyProduct` throws `StoreKitError.unknown` and `product.purchase(options:)` ignores the date. The hosted suite has the test, as a known issue where it cannot pass. |
...
`make integration` and `make integration-ios` do not gate pull requests: a lane that can fail for reasons of its own teaches people to ignore it, and this one depends on StoreKit's test environment, on the runner's Xcode, and on a race or two. But every claim here about what StoreKit does lives in that suite, so "by hand, before a release" was not often enough either. It runs **nightly and on request** (`.github/workflows/integration.yml`), never blocking, with one retry, the result bundle kept, and a red night opening an issue. **Its first run earned its keep**: on a hosted runner — the same macOS, an older Xcode — the suite ran, every test that touches StoreKit worked, and two of this package's *beliefs* failed: backdating, written down as broken on macOS 26, works there (it is Xcode 27.0 that breaks it); and "a grant is announced before it is listed", written down as a law, is a race. **[ran]** From the command line on a Mac it takes about half a minute, and was green every time it was run here. **[ran]**
