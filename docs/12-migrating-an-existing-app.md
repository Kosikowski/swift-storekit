...
2. **Replace the manager with `PurchaseStore` at the composition root.** Make the store once — `StoreLaunch.make(catalogue:)`, which is `AppStoreFront` and a `PurchaseStore` in a release build, and in a debug one can be told to simulate — and apply `.purchaseStore(_:)` at the root ([getting started](02-getting-started.md#the-composition-root)). Remove the old manager's `Transaction.updates` listener in the same change. Do not run both: two listeners compete for the same transactions, and the old one may finish a transaction the new store is waiting to see `[check]`. The demo app stands its own store down while it hosts the real-StoreKit tests for that reason.
