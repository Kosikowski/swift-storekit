...
The first run with Xcode 26.6 found the same rule from the other side. That test environment renewed when told to fail a charge, and gave no grace period when told to give one, in every run. The store said what StoreKit said. `billingRetry`, `gracePeriod` and `habitBillingRetryListing` now expect this with Xcode 26.6 only, which a test learns from the compiler. They excuse it only when StoreKit's own status shows it, and hold the store to that status. With Xcode 27 they are as strict as before **[ran]** ([spike](../spike/README.md#with-xcode-266)).
