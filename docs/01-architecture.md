...
| `PurchaseUI` | Environment entries, a start-at-launch modifier, three buttons: buy, restore, manage subscriptions; and `storeMessages(deferredWhile:showing:)`, which holds Apple's own messages while the app says so, by `StoreMessageReason`. No paywall | SwiftUI; StoreKit in three files, for `@Environment(\.purchase)`, the manage-subscriptions sheet, and `Message.messages` — view-layer calls, which only a view can show, and none that decides anything; AppKit or UIKit in the second, to hear the app become active again. What the messages do is `MessageQueue`, which imports nothing |
...
|---|---|---|
| **Domain** | `@MainActor`, Observation, the store, the role protocols | `ProductID`, `Catalogue`, `CatalogueEntry`, `TrialTerms`, `TrialPeriod`, `TrialStatus`, `SubscriptionGroupID`, `SubscriptionTerms`, `HeldSubscription`, `Renewal`, `AppliedOffer`, `OfferKind`, `OfferPaymentMode`, `OfferID`, `OfferTerms`, `BillingPeriod`, `IntroductoryEligibility`, `WinBackOffer`, `BillingPlan`, `BillingPlanTerms`, `SubscriptionCommitment`, `CommitmentRenewal`, `BundleMembership`, `BundledSubscription`, `NonRenewingTerms`, `NonRenewingPeriod`, `NonRenewingStatus`, `RequestedPurchase`, `SubscriptionStanding`, `Ownership`, `OwnedProduct`, `StoreProduct`, `ProductAccess`, `Standing`, `StandingResolver`, `TransactionUpdate`, `PurchaseOutcome`, `PurchaseCompletion`, `PurchaseOptions`, `RestoreOutcome`, `PurchaseError`, `PurchaseConfirmation`, `PurchaseEvent`, `StoreDiagnosis`, and the `package`-level `Duration.timeInterval` |
